...
def jFitnessFunction(feat, label, X, opts): #适应度函数
    # Default of [alpha; beta]
    ws = [0.9, 0.1]  # [1, 0]  分类结果权重  特征尺寸大小权重

    if "ws" in opts:  # opts 参数可选项
        ws = opts["ws"]

    # Check if any feature exist
    if np.sum(X == 1) == 0:   #如果X的各个分量都不等于1(也即是有效特征值)，所以X中没有特征可供选择，返回0
        return 1
    else:
        # Error rate
...
def jwrapper_KNN(sFeat, label, opts):#计算特征选择结果的错误率   
    if "k" in opts:   #调整K-NN算法的K的大小
        k = opts["k"]
    else:
        k = 5       #默认是5

    kf = KFold(n_splits=5) #使用KFold进行相应的交叉验证操作 默认分为5组进行
    Acc = []
...
def jMultiTaskPSO(feat, label, opts):
    ...
    ub = 1
    thres = 0.5
    c1 = 2  # cognitive factor   自我辨认能力
    c2 = 2  # social factor     社会因子
    c3 = 2  # group social factor  sub_1 从sub_2中学习到的经验权重值
    w = 0.9  # inertia weight   惯性系数
    Vmax = (ub - lb) / 2  # Maximum velocity  最大粒子的飞行速度  这个是什么意思？？？
    ...
    while t <= max_Iter:
        ...
        for i in range(N):
            if k == 0:  # Subpopulation 1
                ...
                valued_features[
                    index
                ] = 0  # Let the features with value less than 0 equal to 0
                ...
                for d in range(dim):
                    ...
                    if valued_features[d] / sum_values > p:
                        ...
                        X[i, d] = 0
            # Fitness
            fit[i] = fun(feat, label, X[i, :] > thres, opts) #调用jfitness函数。其中X[i,:]> thres表示 依据阈值 thres对相应的 维度进行二值化操作
            # Feature vector (new)
            fv_n = X[i, :] > thres
            # Feature vector (old)
            fv_o = Xpb[i, :] > thres
...
def Training(p_name, i):
    ...
    feat = traindata[:, :-1]  #提取所有行 以及除了最后一列的所有列
    label = traindata[:, -1]   #提取所有行 和最后一列
